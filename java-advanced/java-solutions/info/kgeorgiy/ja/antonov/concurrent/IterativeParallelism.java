...
//java -cp . -p . -m info.kgeorgiy.java.advanced.concurrent list info.kgeorgiy.ja.antonov.concurrent.IterativeParallelism
...
public class IterativeParallelism implements ListIP {
     ...
     */
    private <T, U> U verdictWalker(
            int i, List<T> list, Function<Stream<T>, U> assocOp, Function<Stream<U>, U> endResFunction
    ) throws InterruptedException {

        if(mapper != null){
            return endResFunction.apply(mapper.map((l) -> assocOp.apply(l.stream()), separateOnSubLists(i, list)).stream());
        } else {
            return parallelVerdictWalker(i, list, assocOp, endResFunction);
        }
    ...
    private <T> T monoidWalker(
        ...
        return verdictWalker(i, list, operation, operation);
    }
    ...
    public <T> boolean all(
        ...
        return verdictWalker(i, list, (s) -> s.allMatch(predicate), (s) -> s.allMatch(Predicate.isEqual(true)));
    }
